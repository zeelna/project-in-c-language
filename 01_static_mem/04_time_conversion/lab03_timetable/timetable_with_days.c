...
void PrintTimetable(int startHour, int startMin, int nClients, int appLen, int breakLen);

int CalcNextDay(int cMin, int cHour);
...
int main(void)
{
	...
	printf("Using advanced version of the task");
	
    printf("\nWorkday starts at ");
    PrintTime(WORKDAY_START_HOUR, WORKDAY_START_MIN);
    
    printf("\nWorkday ends at ");
    PrintTime(WORKDAY_END_HOUR, WORKDAY_END_MIN);
	...
	printf("Enter break length\n");
	scanf("%d", &breakInput);
  
  
    PrintTimetable(WORKDAY_START_HOUR, WORKDAY_START_MIN, clients, interval, breakInput);
...
//~ int CalcNextDay(int cMin, int cHour, int interval, int breakLen)
int CalcNextDay(int cMin, int cHour)
{
	int nextDayBoolean = 0;
	
	/* endOfHour*/
	if ((cHour >= WORKDAY_END_HOUR) )
	{
		cHour = WORKDAY_START_HOUR;
		cMin = WORKDAY_START_MIN;
		
		/* Assign 1 (True), if we need new Day*/
		nextDayBoolean = 1;	
	}
	return nextDayBoolean;
}
...
void PrintTimetable(int startHour, int startMin, int nClients, int appLen, int breakLen)
{
	...
	//~ int cntDay = 0; // put into arguments?
	int clientIdx = 0; 
	...
	while(1) 	// WHILE TRUE;
	{
		if (clientIdx >= nClients)
		{
			break;
		}
		
		printf("\nDay %d", cntDays);
		cntDays++; // DO NOT FORGET;
		...
		for (i = 0; i < nClients; i++)
		{
			...
			/* end of Day - wrong */
			if (CalcNextDay(endOfMinute, endOfHour) == 1)
			{
				//cntClient += 1;
				currMin = WORKDAY_START_MIN;
				currHour = WORKDAY_START_HOUR;
			...
			printf("\n\tClient %2d: ", clientIdx+1);
			PrintTimeInterval(currHour, currMin, endOfHour, endOfMinute);
			
			/* break */ /* without break */
			currHour = CalcNextHour(endOfMinute, endOfHour, breakLen); //~ startHour = endOfHour;
			currMin = CalcNextMin(endOfMinute, endOfHour, breakLen); //~ startMin = endOfMinute;
			
			
			clientIdx += 1; // DO NOT FORGET
		}
	}
}
