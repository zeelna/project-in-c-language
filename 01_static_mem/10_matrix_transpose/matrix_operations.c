 ...
 * Author:      Frank Christoher Kirch
 * Created:     12.10.2023
 * Modified:    12.10.2023
 *
 * Description: Code for the lab task to switch rows and columns in a
...
int DeleteRow(int matrix[LIMIT_ROWS][LIMIT_COLS], int rows, int cols, int deleteRow)
{
    ...
    /** Separate for-cycle to zero-out values - CAN COMMENT OUT (RISTO) **/
...
void TransposeMatrix(int matrix[LIMIT_ROWS][LIMIT_COLS], int rows, int cols)
{
    ...
    // look this again 1st ternary will overwrite cols!! thus ternary fails. 
