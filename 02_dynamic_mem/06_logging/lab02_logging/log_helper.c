...
#include <stdio.h>
//~ char *pGlobalFilename = globalFilename;
//~ char *globalFilename = "log.txt";
//~ char globalFilename[FILE_NAME_LEN] = "log.txt";


//~ char *globalFilename = "log.txt"; // global variable 
//~ char *globalDefaultLogExtention;
...
{
	puts("Please enter a filename to loggers will be sent");
	putchar('\n');
...
bool IsValidLogExtention(char fileName[LOG_FILE_NAME_LEN])
{
	...
	int extLen = SplitFilenameToGetFileExtention(extention, delimiterIndex);
